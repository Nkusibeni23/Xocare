export default function Background() {
  return (
    <div className="px-4 sm:px-8 md:px-16 max-w-screen-2xl mx-auto">
      <h2 className="font-medium text-black text-center py-6">
        HOW WE SUPPORT SINGLE PARENT FAMILIES
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4">
          <img
            src="https://lh3.googleusercontent.com/pw/ABLVV85U_otcor68MNehm-12PaldldhnaYVk6CeHZxceTmhhKyaacVW632YmURHBG5fA9hgysOZJ_1JIT4D9wtD3fRm5kD7yiPbKidfYaHKni8bFo1MXMawp4_9mVmCIPou9FzNdZYzkddkOB9dgf-LuSis=w1514-h1010-s-no-gm?authuser=0"
            alt=""
            className="w-full rounded-lg object-cover mb-3"
          />
          <h3 className="font-medium mb-3 uppercase">Assist</h3>
          <p className=" text-sm lg:text-lg text-gray-700 font-medium">
            Xocare provides emergency stabilization for families who are in
            danger of losing everything by connecting them to vital resources.
          </p>
          <button className=" p-4 border-2 border-black rounded-lg mt-6 hover:bg-black hover:text-white transition-all duration-500 uppercase text-sm">
            Learn More &rarr;
          </button>
        </div>
        <div className="p-4">
          <img
            src="https://lh3.googleusercontent.com/pw/ABLVV84nJ6fn7uu2pxMuLST543ZKExOA-XIcLOHhtusFSltuW_tCy_ZuL2QplrqCJmO4oWAo1NKsOdyzdbLrXFOI1wwZ5gyRXf3L4XJgPCgSwrK-APOTQ9Mte7tRCFRGB6B9Su1dBPDbLV9FjuPmuKNCxzw=w1514-h1010-s-no-gm?authuser=0"
            alt=""
            className="w-full rounded-lg object-cover mb-3"
          />
          <h3 className="font-medium mb-3 uppercase">Connect</h3>
          <p className=" text-sm lg:text-lg text-gray-700 font-medium">
            Xocare provides single parents with meaningful donations on a weekly
            basis that will assist them in supporting their families
          </p>
          <button className=" p-4 border-2 border-black rounded-lg mt-6 hover:bg-black hover:text-white transition-all duration-500 uppercase text-sm">
            Learn More &rarr;
          </button>
        </div>
        <div className="p-4">
          <img
            src="https://lh3.googleusercontent.com/pw/ABLVV87BwYP7sO5H8nwiYnWPacb6h26kE6Adlz7FVo5oVY0pdwu5ix7tRXYLMTmfNAW__lytqxudv8g4MfBZZsCGLQy86KMRb1gbEtWpmBxSdlaJwcV7ciJ27tJnw7_6PFeGxILBweVj40-MjV5bojyC620=w1514-h1010-s-no-gm?authuser=0"
            alt=""
            className="w-full rounded-lg object-cover mb-3"
          />
          <h3 className="font-medium mb-3 uppercase">Grow</h3>
          <p className=" text-sm lg:text-lg text-gray-700 font-medium">
            Xocare provides training programs for parents to stop financial
            free-fall and learn life skills to successfully raise their
            families.
          </p>
          <button className=" p-4 border-2 border-black rounded-lg mt-6 hover:bg-black hover:text-white transition-all duration-500 uppercase text-sm">
            Learn More &rarr;
          </button>
        </div>
        <div className="p-4">
          <img
            src="https://lh3.googleusercontent.com/pw/ABLVV86PoTeK9A0uCWpC0lmz4e8AjqkeHaBBlX3yCTvxKaHSvyeibMvA1NDAYG3AJC4BQU3vu53bSvdDv2KDEe_BgHItMtgvAYSbaSQtF1JpPHcXoMZ-DKVTV4o9ggadWvzW-32Z5EAzhpPAsr-CzyUVJiM=w1514-h1010-s-no-gm?authuser=0"
            alt=""
            className="w-full rounded-lg object-cover mb-3"
          />
          <h3 className="font-medium mb-3 uppercase">Inspire</h3>
          <p className=" text-sm lg:text-lg text-gray-700 font-medium">
            SMO rewards families who are on a path to independence through
            several celebratory special events through the year.
          </p>
          <button className=" p-4 border-2 border-black rounded-lg mt-6 hover:bg-black hover:text-white transition-all duration-500 uppercase text-sm">
            Learn More &rarr;
          </button>
        </div>
      </div>
    </div>
  );
}
